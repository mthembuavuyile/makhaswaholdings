...
\begin{tikzpicture}[remember picture, overlay]
    ...
    % Full-Bleed Background Project/Building Hero Image
    \node[anchor=center, inner sep=0pt] at (current page.center) {
        \includegraphics[width=\paperwidth, height=\paperheight, keepaspectratio=false]{images/makhaswa-building-hero.jpeg}
    };

    % Deep Navy Semi-Transparent Overlay Tint
    \fill[mhnavy, opacity=0.82] (current page.north west) rectangle (current page.south east);

    % Centered Primary Logo & Typography
    \node[anchor=center] at ([yshift=3.0cm]current page.center) {
        \begin{minipage}{\paperwidth}
            ...
            % Company Name
            {\fontsize{44pt}{48pt}\selectfont\bfseries\color{white}MAKHASWA}\\[0.15cm]
            {\fontsize{26pt}{30pt}\selectfont\bfseries\color{mhbluelight}\makebox[\width][s]{HOLDINGS}}

            \vspace{0.35cm}
            {\fontsize{9pt}{12pt}\selectfont\bfseries\color{white!85}
            CIVIL ENGINEERING \ $\bullet$ \ INFRASTRUCTURE \ $\bullet$ \ BUILDING \ $\bullet$ \ MECHANICAL}
        \end{minipage}
    };

    % Lower Solid Corporate Blue Accent Banner
    \node[anchor=south west, inner sep=0pt] at ([yshift=2.8cm]current page.south west) {
        \begin{tikzpicture}
            \fill[mhblue] (0,0) rectangle (\paperwidth, 1.7cm);
            \fill[mhgold] (0, 1.62cm) rectangle (\paperwidth, 1.7cm);
            \node[anchor=center] at (0.5\paperwidth, 0.85cm) {
                {\fontsize{15pt}{18pt}\selectfont\bfseries\color{white}COMPANY PROFILE}
            };
        \end{tikzpicture}
    };
    ...
    \node[anchor=south] at ([yshift=1.1cm]current page.south) {
        {\fontsize{8.5pt}{11pt}\selectfont\color{white!75}
        CIDB Grade 8 CE PE / 8 ME PE \ $\bullet$ \ Level 1 B-BBEE (135\%) \ $\bullet$ \ ISO 9001 $\bullet$ 14001 $\bullet$ 45001 \ $\bullet$ \ NHBRC Class 5}
    };
...
% =================================================
% PAGE 2 --- LOGO WATERMARK / BRAND DIVIDER SPREAD
% =================================================
\thispagestyle{empty}
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    % Left Blue / Gold Seam
    \fill[mhnavy] (current page.north west) rectangle ([xshift=0.8cm]current page.south west);
    \fill[mhgold] ([xshift=0.8cm]current page.north west) rectangle ([xshift=0.95cm]current page.south west);

    % Large Centered Makhaswa Logo Card (Shifted Upwards)
    \node[anchor=north] at ([yshift=-3.8cm]current page.north) {
        \begin{tcolorbox}[
            enhanced,
            colback=white,
            colframe=mhlightgray,
            ...
            \includegraphics[width=7.5cm, keepaspectratio]{images/logo.jpeg}
        \end{tcolorbox}
    };

    % Brand Statement (Positioned cleanly below the logo card)
    \node[anchor=north] at ([yshift=-10.8cm]current page.north) {
        \begin{minipage}{0.7\paperwidth}
            \centering
            {\fontsize{21pt}{25pt}\selectfont\bfseries\color{mhnavy}BUILDING WITH PURPOSE}\\[0.25cm]
            {\fontsize{15pt}{19pt}\selectfont\bfseries\color{mhblue}\& ENGINEERING PRECISION}\\[0.45cm]
            {\fontsize{12pt}{16pt}\selectfont\itshape\bfseries\color{mhgold}``Live Beyond Measures''}\\[0.55cm]
            {\fontsize{9.5pt}{14.5pt}\selectfont\color{mhtextmuted}
            Delivering high-impact civil, structural, and mechanical infrastructure across South Africa since 2017. Built on rigorous safety standards, technical precision, and shared community value.
            }
        \end{minipage}
    };

    % Bottom Company Legal Notation
    \node[anchor=south, inner sep=0pt] at ([yshift=2.2cm]current page.south) {
        \begin{minipage}{0.6\paperwidth}
            \centering
            {\fontsize{9.5pt}{13pt}\selectfont\bfseries\color{mhnavy}MAKHASWA HOLDINGS (PTY) LTD}\\[0.1cm]
            {\fontsize{8pt}{11pt}\selectfont\color{mhtextmuted}Registration No: 2017/307900/07 (K2017/307900/07) \ $\bullet$ \ Incorporated 17 July 2017}
        \end{minipage}
    };
\end{tikzpicture}
\clearpage
...
% =================================================
% PAGE 3 --- OVERVIEW & TABLE OF CONTENTS
% =================================================
\thispagestyle{empty}
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \fill[mhnavy2] (current page.north west) rectangle (current page.south east);

    % Framing Header & Side Accents
    \fill[mhnavy] (current page.north west) rectangle ([yshift=-2.8cm]current page.north east);
    \fill[mhnavy] ([xshift=-1.8cm]current page.north east) rectangle (current page.south east);

    % Floating Main Content Card
    \node[anchor=center] at ([xshift=-0.5cm, yshift=-0.2cm]current page.center) {
        \begin{tcolorbox}[
            enhanced,
            ...
            % Header
            \begin{center}
                {\fontsize{26pt}{30pt}\selectfont\bfseries\color{mhnavy} OVERVIEW}
                \vspace{0.15cm}
                \textcolor{mhblue}{\rule{3.2cm}{2.2pt}}
            \end{center}

            \vspace{0.6cm}

            % Two-Column Body: Thumbnail Imagery + TOC
            \begin{minipage}[t]{0.40\textwidth}
                ...
                \begin{tcolorbox}[
                    ...
                    \includegraphics[width=\linewidth, height=5.5cm, keepaspectratio=false]{images/makhaswa-building-hero.jpeg}
                \end{tcolorbox}

                \vspace{0.4cm}
                {\fontsize{8.8pt}{12.5pt}\selectfont\color{mhtextmuted}
                Delivering high-tier civil engineering, road surfacing, mechanical pump stations, and commercial infrastructure across South Africa.
                }
            \end{minipage}
            \hfill
            \begin{minipage}[t]{0.54\textwidth}
                \vspace{0.05cm}
                \fontsize{9.2pt}{16pt}\selectfont

                \noindent
                \textbf{\color{mhtext}01\ \ Executive Summary \& National Footprint} \dotfill \textbf{\color{mhnavy}1}\\
                \textbf{\color{mhtext}02\ \ Governance, Leadership \& CR Appointments} \dotfill \textbf{\color{mhnavy}2}\\
                \textbf{\color{mhtext}03\ \ Statutory Registrations \& CIDB Matrix} \dotfill \textbf{\color{mhnavy}3}\\
                \textbf{\color{mhtext}04\ \ Our Journey \& Strategic Milestones} \dotfill \textbf{\color{mhnavy}4}\\
                \textbf{\color{mhtext}05\ \ Core Engineering Disciplines \& Capabilities} \dotfill \textbf{\color{mhnavy}5}\\
                \textbf{\color{mhtext}06\ \ Plant, Fleet \& Technical Software Capacity} \dotfill \textbf{\color{mhnavy}6}\\
                \textbf{\color{mhtext}07\ \ SHEQ Policy, ISO Alignment \& Lab Testing} \dotfill \textbf{\color{mhnavy}7}\\
                \textbf{\color{mhtext}08\ \ Project Portfolio: Civil Infrastructure \& Roads} \dotfill \textbf{\color{mhnavy}8}\\
                \textbf{\color{mhtext}09\ \ Project Portfolio: Buildings, Water \& Reticulation} \dotfill \textbf{\color{mhnavy}9}\\
                \textbf{\color{mhtext}10\ \ CSI \& Socio-Economic Transformation} \dotfill \textbf{\color{mhnavy}10}\\
                \textbf{\color{mhtext}11\ \ Contactable References Directory} \dotfill \textbf{\color{mhnavy}11}\\
                \textbf{\color{mhtext}12\ \ Tender Due Diligence, Warranties \& Contacts} \dotfill \textbf{\color{mhnavy}12}
            \end{minipage}

        \end{tcolorbox}
    };

    % Small Right-Edge Logo Accent Card
    \node[anchor=center] at ([xshift=-0.9cm, yshift=2.0cm]current page.east) {
        \begin{tcolorbox}[
            enhanced,
            colback=white,
            colframe=mhlightgray,
...
\statcard{mhnavy}{01}{Integrity} &
\statcard[10pt]{mhblue}{02}{Excellence} &
\statcard{mhbluebright}{03}{Safety 1st} &
\statcard[10pt]{mhnavy3}{04}{Empowerment} &
\statcard{mhblue}{05}{Sustainability} &
\statcard[10pt]{mhnavy}{06}{Governance} \\
\end{tabularx}
...
\statcard{mhnavy}{L1}{B-BBEE Level} &
\statcard{mhblue}{8 CE/ME}{CIDB Grade} &
\statcard{mhbluebright}{100\%}{Black-Owned} &
\statcard{mhnavy3}{3x}{ISO Standards} \\
\end{tabularx}
...
\noindent
\begin{tabularx}{\textwidth}{X X X X}
\statcard{mhnavy}{2017}{Year Established} &
\statcard{mhblue}{Grade 8}{CE PE / 8 ME PE} &
\statcard{mhbluebright}{Level 1}{B-BBEE Contributor} &
\statcard{mhnavy3}{100\%}{Black-Owned Firm} \\
\end{tabularx}
...
\statcard{mhnavy}{01}{Planning} &
\statcard{mhblue}{02}{Procurement} &
\statcard{mhbluebright}{03}{Execution} &
\statcard{mhnavy3}{04}{QA / QC} &
\statcard{mhblue}{05}{Handover} \\
\end{tabularx}
...
\statcard{mhnavy}{Multi-Fleet}{Deployment Logistics} &
\statcard{mhblue}{Pr.Eng / OHS}{Site Supervisory Control} &
\statcard{mhbluebright}{Multi-Site}{Concurrent Deployments} \\
\end{tabularx}
...
\noindent
\begin{tabularx}{\textwidth}{X X X X}
\statcard{mhnavy}{ISO Standards}{9001 $\bullet$ 14001 $\bullet$ 45001} &
\statcard{mhblue}{Zero Harm}{Safety Target} &
\statcard{mhbluebright}{OHS Act}{85 of 1993 Compliant} &
\statcard{mhnavy3}{Daily}{Toolbox Talks} \\
\end{tabularx}
